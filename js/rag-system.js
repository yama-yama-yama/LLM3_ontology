...
displayRAGResult(result) {
    ...
    function getSimilarityColor(similarity) {
        if (similarity > 0.8) return '#4CAF50';      // 関連度高い
        if (similarity > 0.5) return '#FF9800';      // 中ぐらい
        return '#9E9E9E';                            //　無し
    }

    container.innerHTML = `
         <div style="margin: 20px 0;">
             <h3 style="color: #000000ff;">回答:</h3>
             <p style="padding: 15px; background: #f5f5f5; border-radius: 5px;">
                 ${result.response}
             </p>
             <h4 style="margin-top: 20px;">参考文献:</h4>
